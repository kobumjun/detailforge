...
1. SQL Editor에서 `supabase/migrations/001_initial.sql` 전체를 실행합니다.
...
- `generations`, `credit_logs`, `payment_orders`: 본인 데이터만 읽기/쓰기.
...
- `SUPABASE_SERVICE_ROLE_KEY` (PNG 업로드 후 서명 URL, 스토리지 업로드)
- `APP_URL` (로컬/배포 URL)
...
src/app/
  ...
  api/generations/[id]/preview|export
src/lib/
